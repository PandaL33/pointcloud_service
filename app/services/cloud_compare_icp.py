...
class CloudCompareIcp:
    ...
    def load_point_cloud(self, file_path: str, voxel_size: Optional[float] = None) -> np.ndarray:
        """加载点云文件
        ...
        # 如果指定了voxel_size，则进行体素下采样
        if voxel_size is not None and voxel_size > 0:
            logger.info(f"对点云进行体素下采样，体素大小: {voxel_size}")
            downsampled_pcd = pcd.voxel_down_sample(voxel_size)
            points = np.asarray(downsampled_pcd.points)
            logger.info(f"下采样前点数: {len(pcd.points)}, 下采样后点数: {len(points)}")
    ...
    def registrator(self, args):
        ...
        try:
            ...
            if voxel_size is not None and voxel_size > 0:
                ...
                aligned_cloud_voxel = aligned_cloud
                
            logger.info(f"加载参考点云: {args.reference}")
            reference_cloud = self.load_point_cloud(args.reference, None)
            ...
            transformed_points, transform_matrix, fitness, iterations = self.cloud_compare_icp(
                aligned_cloud_voxel, reference_cloud,
            ...
            return transform_matrix, fitness, iterations, icp_pcd
    ...
    def run_registrator(self, aligned_file, reference_file, output_file, voxel_size=None):
        """
        执行主函数
        :param aligned_file: 待配准点云文件路径
        :param reference_file: 参考点云文件路径
